...
class CartPoleEnv:
    ...
    def __init__(self, fog_id, ori_un_reliability, d_size, d_reliability, fog_id_cur_capacity_dict,
        ...
        # 动作空间：选择的fog_id 选择的可靠性等级，二元组
        # 定义一个 MultiDiscrete 类型的离散空间
        self.action_space = spaces.Discrete(FOG_NUM * R_RANK_NUM)

        # 当前状态空间，是一个三维数组 observation ：max{f2f距离}（时延），当前已达到的数据不可靠性，已备份的副本数
        # 三个维度
        # 1：初始为0 的非负整数
        # 2：初始为self.ori_un_r的 0.0~self.ori_un_r间浮点数
        # 3：初始为1的非负整数
    ...
    def step(self, a):
        ...
        # 当前时延
        # 1. 内存剩余率 越大越优先
        # if self.fog_c_r_r_mem_rank_max_min_dict[r_rank][fog_id - 1][0] == 2:
        #     mem_normalized = -1000
        # # elif self.fog_c_r_r_mem_rank_max_min_dict[fog_id][1] == 2:
        # #     mem_normalized = -50
        # else:
        ...
        if temp_un_r <= 1 - self.d_reliability:
            # 可靠性已经满足
            return self._get_obs(), -cost, True, False, {}
        # 未满足，继续
